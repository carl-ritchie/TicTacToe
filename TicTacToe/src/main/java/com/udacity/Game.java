...
public class Game {
    ...
    private int turns; // counts the number of turns that have been played
    ...
    public String checkGameWinner(char[][] grid) {
        ...
        // checks every row for 3 'x' or 'o' in a row
        for (int j = 0; j < 3; j++) {
            if (grid[0][j] == grid[1][j] && grid[1][j] == grid[2][j]) {
                if (grid[0][j] != '-') {
                    String winner = Character.toUpperCase(grid[0][j]) + " wins";
                    result = winner;
                }
            }
        }

        // checks every column for 3 'x' or 'o' in a row
        for (int i = 0; i < 3; i++) {
            if (grid[i][0] == grid[i][1] && grid[i][1] == grid[i][2]) {
                if (grid[i][0] != '-') {
                    String winner = Character.toUpperCase(grid[i][0]) + " wins";
                    result = winner;
                }
            }
        }

        // checks upper left to lower right diagonal
        if (grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2]) {
            if (grid[0][0] != '-') {
                String winner = Character.toUpperCase(grid[0][0]) + " wins";
                result = winner;
            }
        }

        // checks lower left to upper right diagonal
        if (grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0]) {
            if (grid[0][2] != '-') {
                String winner = Character.toUpperCase(grid[0][2]) + " wins";
                result = winner;
            }
        }

        // checks for 9, err, 18? turns
        if (turns == 18) {
            // String tie = "Tie";
            result = "Tie";
        }
